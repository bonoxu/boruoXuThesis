...
In \Chapter{chap:Reconstruction}, the software for event simulation, event reconstruction, and event analysis is  discussed. The event reconstruction focus on \pandora, a world-leading pattern-recognition software for particle flow calorimetry. \pandora aims to reconstruct individual particles in the event. Jet algorithms is also used, followed by a discussion on  multivariate analysis, where different fitting models, optimisation, and overfitting are discussed. The multivariate analysis is used extensively in the \Chapter{chap:Photon}, \Chapter{chap:Tau}, and \Chapter{chap:DoubleHiggs}.
