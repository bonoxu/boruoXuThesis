\chapter{Detectors for the Future Electron-positron Particle Colliders}
\label{chap:Detector}
...
Two leading candidates for next generation electron-positron linear colliders are the International Linear Collider (\ILC) \cite{Brau:2007zza}, and the Compact Linear Collider (\CLIC) \cite{Linssen:2012hp}. This chapter provides an overview of the two colliders, followed by the physics program at these colliders, the detector requirement driven my the physics program, the particle flow calorimetry, the detectors for the \ILC and the \CLIC.

\section{The \ILC}

The \ILC is a high-luminosity electron-positron linear collider with centre-of-mass energies from 200\,GeV up to 1\,TeV. The machine would be built at different stages. The first stage would have a centre-of-mass energy of 250/350\,GeV. The second stage would have a centre-of-mass energy of  500\,GeV with a possible upgrade to 1\TeV. A layout of the collider complex is shown in \Figure{fig:detectorILC}.  Two detector concepts were developed for the \ILC, the International Large Detector (\ILD)\cite{Abe:2010aa}, and the Silicon Detector (\SiD) \cite{Aihara:2010zz}, shown in \Figure{fig:detectorILDSiD}.
...
The other potential next-generation electron-positron linear collider \CLIC \cite{Linssen:2012hp}, has a higher reach of the centre-of-mass energy, up to 3\,TeV. \CLIC is designed to built in stages as well.  The first stage, with a centre-of-mass energy of 380\,GeV. The final stage of a centre-of-mass energy of 3\,TeV. The second stage has a centre-of-mass energy of 1.4\,TeV, which bridges between the first stage and the final stage. The layout of the \CLIC complex at final stage of a centre-of-mass of energy of  3\,TeV is shown in \Figure{fig:detectorCILC}.


The \ILC and the \CLIC share some common design features. Both colliders will be the linear colliders as oppose to the circular colliders like the \LHC. Detectors for both colliders will use the high granularity particle flow calorimetry \cite{Baer:2013cma,Linssen:2012hp}. The conceptual designs of two the \CLIC detectors ware based on the \ILC detectors \cite{Linssen:2012hp}. One of the major differences of two colliders is operating energies. There are significant beam related backgrounds at the \CLIC. The \ee incoherent pair background has a major influence on the design of the inner region of the detector and the forward region. The pile-up of 3.2 \ggHad events per bunch is significant and needs to be removed for physics analyses. Also, the \CLIC beam contains 312 bunch trains with a train repetition rate of 50\,Hz. The bunches are  separated by 0.5\,ns within each bunch train. The short timing separation between bunches suggests that the detector will integrate over a number of bunch crossings. This, together with the significant \ggHad backgrounds, has a significant impact on the detector design.
...
% Due to the similarities of the two linear collider programs, the development with the \CLIC detector concepts started with the \ILC detector concepts. Two  \CLIC detector concepts, the \CLICILD and the \CLICSiD, are developed based on the \ILD and the \SiD, respectively.
...
The promising candidates for the future electron-positron collider, the \ILC and the \CLIC, share some common advantages over the current hadron collider, the \LHC. Those advantages include: i) Events in the \ee collider will be cleaner than those in the hadron collider. In the \LHC, many proton-proton collisions per bunch crossing are expected, generating hundreds of particles from parton collisions. In the \ee collider, the main source of background comes from photon-photon collisions. Depending on operating energy and scheme, there will be a few of these collisions per bunch crossing. Particles produced from these collisions are mainly in the forward direction, which can be identified relatively easily. ii) Electroweak interactions in the \ee collider will be democratic as the photon couples to all particles in and beyond Standard Model equally. The production of pairs of all particles will be at a similar rate. In the \LHC, the non-perturbative strong interaction is the main channel for the particle production. Besides, only a fraction of protons would collide. Heavy particles has a lower production rate than light ones, as the parton distributions fall sharply for a composite object like a proton. iii) Calculation in the \ee collider requires less computational resources than it requires in the \LHC. In the \LHC, calculation of the cross section depends on quantum chromodynamics  and the proton structure function, which has a large systematic error. In the \ee collider, initial particles, \Pep and \Pem, can be treated as point-like particles, interacting through electroweak only. A high theoretical precision level could be reached. iv) The physics in the \ee collider can be studied in details. Without complicated underlying events, complete events can be reconstructed. Momenta of leptons and quarks can be calculated by kinematic fitting. Also polarised beams of electrons and positrons with known initial and final polarisation states could be used to aid the study.

The physics program for the \CLIC and the \ILC, which is a driving force for the detector design, share some common goals. At a centre-of-mass energy of 250\,GeV, the collider operates as a Higgs factory, allowing measurements of precision Higgs couplings via channels like \HepProcess{\ee \to \PZ \PHiggs}. At a centre-of-mass energy of 350\,GeV, the collider can continue to measure Higgs couplings, as well as to measure top quark mass and coupling with channels such as \HepProcess{\ee \to \Ptop \APtop}.  At a centre-of-mass energy of 1\,TeV and beyond, the collider would be able to produce rare Higgs decay, allowing measurements of Higgs self-coupling, probing composite Higgs sector, and searching for supersymmetry particles.
...
Many interesting physics processes at a future linear collider can be described as multi-jet final states, often with charged leptons or missing momentum associated with neutrinos. The reconstruction of the invariant masses of two or more jets is critical for event reconstruction and event selection. At \LEP, kinematic fitting \cite{Thomson:2004zx} allowed precise invariant mass reconstruction. At a future collider, reconstruction invariant mass of multi jets for final states with missing momentum will rely heavily on the intrinsic jet energy resolution of the detector.


One goal for jet energy resolution at a future linear collider it is to be able to separate  \PW and \PZ bosons, by reconstructing the invariant mass via quark-jets using the hadronic decay channel. The idealised reconstructed \PW and \PZ boson mass distribution of different assumed jet mass resolutions is shown in \Figure{fig:detectorWZseparation}. As the invariant mass resolution is comparable to the gauge boson widths, i.e. $\sigma_m/m\approx\Gamma_{\PW}/m_{\PW}\approx\Gamma_{\PZ}/m_{\PZ}$, a separation of 2.5$\sigma$ implies a jet energy resolution of 3.5\% \cite{Thomson:2009rp} for a range of jet energies from 50\,GeV to 1\,TeV.
...
This level of precision,  a jet energy resolution of 3.5\%, is unlikely to be achieved with a traditional calorimeter design. Traditionally, jet energies is measured as a sum of energies deposited in the electromagnetic  (\ECAL)  and hadronic calorimeter (\HCAL), giving a jet energy resolution of the form
\begin{equation}
\frac{\sigma_{E}}{E} = \frac{\alpha}{\sqrt{E(\text{GeV})}}\oplus\beta .
\end{equation}
The stochastic term $\alpha$ is typically greater than 60\% \cite{Abe:2010aa,Behnke:2013lya}, and the constant term $\beta$ is of the order of a few percents \cite{Abe:2010aa,Behnke:2013lya}. To achieve a the jet energy resolution of 3.5\% or better, the stochastic term should be less than 30\% with a small constant term, which is unlikely to be achieved by a traditional calorimeter.
...
The particle flow approach to calorimetry improves the jet energy resolution by fully reconstructing  all visible particles in the detector. The jet energy is the sum of energy of individual particles, where the energy of the charge particles are measured in the tracking detectors, and the energy of neutral particles are measured in calorimeters. In this approach, the hadronic calorimeter only measures about 10\% of the jet energy, which would greatly improve the overall jet energy measurement.
...
In the particle flow approach to calorimetry, the sum of calorimeter energies is replaced by a complex pattern-recognition problem, the Particle Flow reconstruction Algorithm (PFA). Detailed simulation of the \ILC and the \CLIC detector concepts using the \pandora \cite{Thomson:2009rp,Marshall:2012ry} particle flow reconstruction algorithms have demonstrated that a jet energy resolution of approximately 3\% an be achieved for jet energies in the range of 100\,GeV to 1\,TeV.
...
Two detector concepts have been developed for the \ILC. The motivation for two detectors is to have multiple independent measurements within one collider for cross-checking, complementary measurements, and competition between collaborations. The two detectors are both designed to be general purpose detectors. The Silicon Detector, \SiD, is a compact detector with a large magnetic filed of 5\,T. It uses silicon tracking modules. The second detector, the International Large Detector, \ILD, is a larger detector with a time projection chamber as the main tracking unit.
...
\FIGURE{fig:ILD} shows the longitudinal cross section of top quadrant of the \ILD detector concept. From the interaction point (\IP) outwards, there is a tracking system comprising a large time projection chamber (\TPC) augmented with silicon tungsten layers, highly granular electromagnetic calorimeters (\ECAL) and hadronic calorimeters (\HCAL), muon chambers, forward calorimeters (\FCAL), magnetic coils and iron yokes. The key parameters of the \ILD are listed in \Table{tab:ILDvsCLICILD}.
...
A \TPC tracking detector has several advantages: a) Tracks can be measured with a large number of three-dimensional ($r,\phi,z$) spatial points. The continuous tracking allows precise reconstruction of tracks, which is important for particle identification via the specific energy loss $dE/dx$; b) the \TPC uses a minimum amount of material, which minimises the effects due to the $10^3$ beamstrahlung photons per bunch-crossing in the barrel region \cite{Vogel:2008zza}. The \TPC is optimised for point resolution and minimum material, as required for the best calorimeter performance and the best particle flow performance.
...
The Silicon-Tungsten sampling electromagnetic calorimeters in the \ILD consist of a nearly cylindrical barrel and two end cap systems, optimised for particle flow. The fine granular \ECAL is located inside the \HCAL. The \ECAL measures photon energies and separates photons from other particles. The \ECAL also hosts the first part of the hadronic showers and greatly assists the separation of hadronic showers. \FIGURE{fig:detectorILDECAL2} shows the position of the electromagnetic calorimeter in the \ILD detector and the trapezoidal form of the modules.
