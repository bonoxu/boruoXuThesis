...
One of the main objectives of future linear colliders is to be able to separate  \PW and \PZ bosons by reconstructing their invariant masses via quark-jets using the hadronic decay channel. The idealised reconstructed \PW and \PZ boson mass distributions for different jet mass resolutions are shown in \Figure{fig:detectorWZseparation}. As the invariant mass resolution is comparable to the gauge boson widths, i.e. $\sigma_m/m\approx\Gamma_{\PW}/m_{\PW}\approx\Gamma_{\PZ}/m_{\PZ}$, a separation of 2.5$\sigma$ in the mass distributions implies a jet energy resolution of 3.5\% \cite{Thomson:2009rp} for a range of jet energies from 50\,GeV to 1\,TeV.
...
As shown in \Table{tab:detectorJetComponent}, assuming 30\% of the jet energy (photon energy) is measured with ${\sigma_{E}}/{E} = 15\% / \sqrt{E(\text{GeV})}$, and 10\% of the jet energy (hadron energy) is measured with ${\sigma_{E}}/{E} = 55\% / \sqrt{E(\text{GeV})}$ \cite{Behnke:2013lya}, a jet energy resolution of ${\sigma_{E}}/{E} = 19\% / \sqrt{E(\text{GeV})}$ can be obtained. This satisfies the jet energy resolution requirement for separating \PW and \PZ bosons via their hadronic decays, which requires a jet energy resolution of 3.5\% or better. In reality, this level of performance is unattainable due to incorrect association of energy deposits to particles. Results from imperfect reconstruction rather than the intrinsic detector performance limit the performance of particle flow calorimetry \cite{Thomson:2009rp} at jet energies beyond tens of GeVs
...
\caption
{The longitudinal cross section of top quadrant of the \ILD, taken from  \cite{Behnke:2013lya}. From the interaction point (\IP) outwards, the design includes: a tracking system comprising a large time projection chamber (\TPC) augmented with silicon tungsten layers; highly granular electromagnetic calorimeters (\ECAL) and hadronic calorimeters (\HCAL);  forward calorimeters (\FCAL); a superconducting solenoid; and muon chambers embedded within the iron return yokes. Dimensions are in units of mm.}
\label{fig:ILD}
\end{figure}
...
\caption
{The longitudinal cross section of top quadrant of the \CLICILD detector concept, taken from \cite{Linssen:2012hp}. From interaction point (\IP) outwards, the design includes: a tracking system comprising a large time projection chamber (\TPC) augmented with silicon tungsten layers; highly granular electromagnetic calorimeters (\ECAL) and hadronic calorimeters (\HCAL);  forward calorimeters (\FCAL); a superconducting solenoid; and muon chambers embedded within the iron return yokes.}
    \label{fig:CLIC_ILD}
\end{figure}
