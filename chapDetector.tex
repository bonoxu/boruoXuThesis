...
The \ILC and  \CLIC share some common  features. Both colliders will be the linear colliders as oppose to the circular colliders, like the Large Hadron Collider (\LHC). Detectors for both colliders will use the high granularity particle flow calorimetry \cite{Behnke:2013lya,Linssen:2012hp}. One major difference between the two colliders is the operating energy. Due to a higher centre-of-mass energy at \CLIC, there are significant beam related backgrounds. The \ee incoherent pair background has a major influence on the design of the inner region  and the forward region of the detectors \cite{Linssen:2012hp}. The pile-up of 3.2 \ggHad events per bunch, also integrating over 60 bunch crossings,  need to be mitigated for physics analyses  \cite{Linssen:2012hp}.  Another difference between the \ILC and  \CLIC is that the timing separation between bunches is much shorter at \CLIC. The \CLIC beam contains 312 bunch trains with a train repetition rate of 50\,Hz,  separated by 0.5\,ns between each bunch train. 

%The short timing separation between bunches suggests that the detector will integrate over a number of bunch crossings.
...
One goal of jet energy resolution at a future linear collider is to be able to separate  \PW and \PZ bosons, by reconstructing the invariant mass via quark-jets using the hadronic decay channel. The idealised reconstructed \PW and \PZ boson masses distributions of different assumed jet mass resolutions are shown in \Figure{fig:detectorWZseparation}. As the invariant mass resolution is comparable to the gauge boson widths, i.e. $\sigma_m/m\approx\Gamma_{\PW}/m_{\PW}\approx\Gamma_{\PZ}/m_{\PZ}$, a separation of 2.5$\sigma$ in the masses distributions implies a jet energy resolution of 3.5\% \cite{Thomson:2009rp} for a range of jet energies from 50\,GeV to 1\,TeV.
...
\caption
{Ideal \PW/\PZ boson mass separation as a function of jet mass resolution obtained using a Gaussian smearing of Breit$-$Wigner distribution, taken from \cite{Linssen:2012hp}.}
...
{The forward calorimeters of the \ILD, taken from \cite{Behnke:2013lya}. The \LumiCAL, the \BeamCAL, and the \LHCAL are the luminosity calorimeter, the beam calorimeter, and the forward hadronic calorimeter, respectively.}
...
\hline
 & &  \ILD & \CLICILD \\
\hline
\multirow{4}{*}{\LumiCAL} & geometrical acceptance (mrad) & 31 - 77 & 38 - 110 \\
& fiducial acceptance (mrad) & 41 - 67 & 44 - 80 \\
& z (start) (mm) & 2450 & 2654 \\
& number of layers (W + Si) & 30 & 40 \\
\hline
\multirow{4}{*}{\BeamCAL} & geometrical acceptance (mrad)& 5 - 40 & 10 - 40 \\
...
\begin{figure}[tbph]
\centering
    \includegraphics[width=0.45\textwidth]{ILD/HCALoptimise}
\caption
{The single jet energy resolution as a function of the hadronic calorimeter scintillator cell sizes, with different total jet energy using \eeZuds, at barrel region, taken from \cite{Behnke:2013lya}.}
