...
Using the cuts for photon$-$photon-fragment with fragment energy $<$ 1\,GeV as an example,  each set of logics for merging fragments is discussed. Photon$-$fragment pair passing any one set of cuts will be merged.
...
If a photon has the  topology of multiple spatially closed photons, the parent photon will be split into several daughter photons. Extra care should be taken if the parent photon is close to a track projection onto the front of the \ECAL. \TABLE{tab:photonPhotonSplitting} lists the cuts used in the algorithm.
